    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex space-x-3">
          ...
          >
            {isLoading ? 'Processing...' : identity ? 'Update Identity' : 'Create Identity'}
          </button>
          ...
          {identity && (
            <button
              ...
              disabled={isLoading}
              className={`px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              Delete Identity
            </button>
          )}
